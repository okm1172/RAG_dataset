...
struct standard_dataset_descriptor_t
                             ...
                             const std::uint32_t query_smem_buffer_length)
  {
    for (unsigned i = threadIdx.x; i < query_smem_buffer_length; i += blockDim.x) {
      ...
      if (i < dim) {
        // printf("COPY QUERY : %.6f\n",spatial::knn::detail::utils::mapping<QUERY_T>{}(dmem_query_ptr[i]));
...
#pragma unroll
          for (uint32_t v = 0; v < vlen; v++) {
            ...
            //   printf("threadIdx.x : %u, e : %u, v : %u, query : %.6f, node : %.6f\n", threadIdx.x, e, v, d, dl_buff[e].val.data[v]);
            // }
