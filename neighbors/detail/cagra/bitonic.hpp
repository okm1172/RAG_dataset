...
#pragma unroll
          ...
          // if (blockIdx.y == 107 && threadIdx.x < 32 && N == 1) {
          //   printf("query_num %u range %u in warp N %u\n", blockIdx.y, range, N);
          //   for (std::uint32_t debug_for = 0 ; debug_for < N; debug_for++) {
          //     printf("threadIdx.x = %u, b = %u, c = %u, v[%u] = %u\n", threadIdx.x, b, c, debug_for, v[debug_for]);
          //     printf("threadIdx.x = %u, b = %u, c = %u, k[%u] = %f\n", threadIdx.x, b, c, debug_for, k[debug_for]);
          //   }
          // }
          // if (blockIdx.y == 191 && threadIdx.x < 32 && N == 4) {
          //   printf("query_num %u range %u in warp N %u\n", blockIdx.y, range, N);
          //   for (std::uint32_t debug_for = 0 ; debug_for < N; debug_for++) {
          //     printf("threadIdx.x = %u, b = %u, c = %u, v[%u] = %u\n", threadIdx.x, b, c, debug_for, v[debug_for]);
          //     printf("threadIdx.x = %u, b = %u, c = %u, k[%u] = %f\n", threadIdx.x, b, c, debug_for, k[debug_for]);
...
#pragma unroll
      ...
      //   printf("query_num %u range %u in warp N %u\n", blockIdx.y, range, N);
      //   for (std::uint32_t debug_for = 0 ; debug_for < N; debug_for++) {
      //     printf("threadIdx.x = %u, c = %u, v[%u] = %u\n", threadIdx.x, c, debug_for, v[debug_for]);
      //     printf("threadIdx.x = %u, c = %u, k[%u] = %f\n", threadIdx.x, c, debug_for, k[debug_for]);
      //   }
      // }
      // if (blockIdx.y == 191 && threadIdx.x < 32 && N == 4) {
      //   printf("query_num %u range %u in warp N %u\n", blockIdx.y, range, N);
...
struct warp_merge_core<K, V, 2, warp_size> {
  _RAFT_DEVICE inline void operator()(K k[2], V v[2], const std::uint32_t range, const bool asc)
  {
    ...
    if (range == 1) {
      const auto p = ((lane_id & 1) == 0);
      swap_if_needed(k[0], v[0], k[1], v[1], p);
      // if (blockIdx.y == 107 && threadIdx.x < 32) {
      //   printf("query_num %u range %u in warp N 2\n", blockIdx.y, range);
      //   for (std::uint32_t debug_for = 0 ; debug_for < 2; debug_for++) {
      //     printf("query_num %u threadIdx.x = %u, v[%u] = %u\n", blockIdx.y, threadIdx.x, debug_for, v[debug_for]);
      //     printf("query_num %u threadIdx.x = %u,  k[%u] = %f\n", blockIdx.y, threadIdx.x, debug_for, k[debug_for]);
      //   }
      // }
      return;
    }
...
#pragma unroll
      for (std::uint32_t i = 0; i < N; i++) {
        swap_if_needed(k[i], v[i], c, p);
        // if (blockIdx.y == 107 && threadIdx.x < 32) {
        //   printf("query_num %u range %u in warp N 1\n", blockIdx.y, range);
        //   for (std::uint32_t debug_for = 0 ; debug_for < 2; debug_for++) {
        //     printf("query_num %u threadIdx.x = %u, b = %u, c = %u, v[%u] = %u\n", blockIdx.y, threadIdx.x, b, c, debug_for, v[debug_for]);
        //     printf("query_num %u threadIdx.x = %u, b = %u, c = %u, k[%u] = %f\n", blockIdx.y, threadIdx.x, b, c, debug_for, k[debug_for]);
        //   }
        // }
      }
    }
    ...
    //   printf("query_num %u range %u in warp N 1\n", blockIdx.y, range);
    //   for (std::uint32_t debug_for = 0 ; debug_for < 2; debug_for++) {
    //     printf("query_num %u threadIdx.x = %u, b = %u, v[%u] = %u\n", blockIdx.y, threadIdx.x, b, debug_for, v[debug_for]);
    //     printf("query_num %u threadIdx.x = %u, b = %u, k[%u] = %f\n", blockIdx.y, threadIdx.x, b, debug_for, k[debug_for]);
    //   }
    // } else if(blockIdx.y == 74 && threadIdx.x < 32) {
    //   printf("query_num %u range %u in warp N 1\n", blockIdx.y, range);
    //   for (std::uint32_t debug_for = 0 ; debug_for < 2; debug_for++) {
    //     printf("query_num %u threadIdx.x = %u, b = %u, v[%u] = %u\n", blockIdx.y, threadIdx.x, b, debug_for, v[debug_for]);
    //     printf("query_num %u threadIdx.x = %u, b = %u, k[%u] = %f\n", blockIdx.y, threadIdx.x, b, debug_for, k[debug_for]);
...
struct warp_merge_core<K, V, 1, warp_size> {
  _RAFT_DEVICE inline void operator()(K k[1], V v[1], const std::uint32_t range, const bool asc)
  {
    ...
    for (std::uint32_t c = b / 2; c >= 1; c >>= 1) {
      ...
      // if ((blockIdx.y == 107 || blockIdx.y == 391) && threadIdx.x < 32) {
      //   printf("query_num %u range %u in warp N 1\n", blockIdx.y, range);
      //   for (std::uint32_t debug_for = 0 ; debug_for < 1; debug_for++) {
      //     printf("query_num %u threadIdx.x = %u, b = %u, c = %u, v[%u] = %u\n", blockIdx.y, threadIdx.x, b, c, debug_for, v[debug_for]);
      //     printf("query_num %u threadIdx.x = %u, b = %u, c = %u, k[%u] = %f\n", blockIdx.y, threadIdx.x, b, c, debug_for, k[debug_for]);
      //   }
      // }
