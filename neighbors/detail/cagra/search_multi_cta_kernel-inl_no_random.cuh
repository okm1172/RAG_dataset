...
__launch_bounds__(1024, 1) RAFT_KERNEL search_kernel(
  ...
  std::uint32_t* const distance_calculation_count)
{
  ...
  cudaStream_t stream)
{
  ...
  auto distance_calculation_count = raft::make_device_vector<uint32_t>(handle, num_queries);
  auto distance_calculation_count_view = raft::make_device_vector_view<uint32_t, uint32_t>(distance_calculation_count.data_handle(), num_queries);
  kernel<<<grid_dims, block_dims, smem_size, stream>>>(topk_indices_ptr,
                                                       ...
                                                       sample_filter,
                                                       metric,
                                                       distance_calculation_count_view.data_handle()
                                                      );
}
