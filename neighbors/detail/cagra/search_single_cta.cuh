...
struct search : search_plan_impl<DATASET_DESCRIPTOR_T, SAMPLE_FILTER_T> {
  ...
  void operator()(raft::resources const& res,
                  ...
                  INDEX_T* const data_indices_ptr,       // [num_queries, topk]
                  DISTANCE_T* const data_distances_ptr  // [num_queries, topk]
                  )
  {
    cudaStream_t stream = resource::get_cuda_stream(res);
    std::cout<<"thread_block_size : " << static_cast<uint32_t>(thread_block_size) << std::endl;
    std::vector<DISTANCE_T> query_debug(512);
    raft::copy(query_debug.data(), queries_ptr, 512, resource::get_cuda_stream(res));
    std::cout << "OPERATOR()" << query_debug[0] <<" "<<query_debug[1]<< std::endl;
