  ...
  bool construct_index_with_dataset                                       = true)
{
  ...
  if (params.build_algo == graph_build_algo::IVF_PQ) {
    ...
    if (!nn_descent_params) {
      ...
      // nn_descent_params->return_distances          = true; // 이렇게 해야 nnd::build가 수행되는데.. distances를 return하지 않으면.. 어떻게 trace를 하지? 뭐.. CAGRA에선 크게 상관없으니 일단 넘어갑시다.
    }
    // cudaProfilerStart();
