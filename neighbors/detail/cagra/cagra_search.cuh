...
void search_main_core(
  ...
  raft::device_matrix_view<typename DatasetDescriptorT::INDEX_T, int64_t, row_major> neighbors_data,
  raft::device_matrix_view<typename DatasetDescriptorT::DISTANCE_T, int64_t, row_major> distances_data,
  ...
  raft::distance::DistanceType metric = raft::distance::DistanceType::L2Expanded)
{
  ...
  std::cout << "MAX_QUERIES" << max_queries << std::endl;
  std::cout << "QUERIES EXTENT 0" << queries.extent(0) << std::endl;
  ...
  for (unsigned qid = 0; qid < queries.extent(0); qid += max_queries) {
    ...
    auto neighbors_data_ptr = reinterpret_cast<typename DatasetDescriptorT::INDEX_T*>(neighbors_data.data_handle());
    auto distances_data_ptr = distances_data.data_handle();
    // todo(tfeher): one could keep distances optional and pass nullptr
    const auto* _query_ptr = queries.data_handle() + (query_dim * qid);
    std::vector<typename DatasetDescriptorT::DATA_T> query_debug(queries.extent(0) * queries.extent(1));
    raft::copy(query_debug.data(), _query_ptr, queries.extent(0) * queries.extent(1), resource::get_cuda_stream(res));
    std::cout << "SEARCH_MAIN_CORE" << query_debug[0] <<" "<<query_debug[1] << std::endl;
...
void launch_vpq_search_main_core(
  ...
  raft::device_matrix_view<InternalIdxT, int64_t, row_major> neighbors_data,
  raft::device_matrix_view<DistanceT, int64_t, row_major> distances_data,
  ...
  if (vpq_dset->pq_bits() == 8) {
    if (vpq_dset->pq_len() == 2) {
      ...
      dataset_desc_t dataset_desc(vpq_dset->data.data_handle(),
                                  ...
                                  vpq_dset->dim());
      search_main_core(
        res, params, dataset_desc, graph, queries, neighbors, distances, neighbors_data, distances_data, sample_filter, metric);
