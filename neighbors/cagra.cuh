...
void search_with_filtering(raft::resources const& res,
                           ...
                           raft::device_matrix_view<IdxT, int64_t, row_major> neighbors_data,
                           raft::device_matrix_view<float, int64_t, row_major> distances_data,
                           CagraSampleFilterT sample_filter = CagraSampleFilterT())
{
  ...
  std::vector<T> query_debug(queries.extent(0) * queries.extent(1));
  raft::copy(query_debug.data(), queries_internal.data_handle(), queries.extent(0) * queries.extent(1), resource::get_cuda_stream(res));
  std::cout << "SEARCH_WITH_FILTERING" << query_debug[0] <<" "<<query_debug[1] << std::endl;
  ...
  auto neighbors_internal_data = raft::make_device_matrix_view<internal_IdxT, int64_t, row_major>(
    reinterpret_cast<internal_IdxT*>(neighbors_data.data_handle()),
    neighbors_data.extent(0),
    neighbors_data.extent(1)
  );
  auto distances_internal_data = raft::make_device_matrix_view<float, int64_t, row_major>(
    distances_data.data_handle(), distances_data.extent(0), distances_data.extent(1));

  return cagra::detail::search_main<T, internal_IdxT, CagraSampleFilterT, IdxT>(
    res, params, idx, queries_internal, neighbors_internal, distances_internal, neighbors_internal_data, distances_internal_data, sample_filter);
}
          ...
          )
{
  ...
  return cagra::search_with_filtering<T, IdxT, none_filter_type>(
    res, params, idx, queries, neighbors, distances, neighbors_data, distances_data, none_filter_type{});
}
